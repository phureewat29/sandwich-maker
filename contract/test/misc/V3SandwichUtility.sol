...
library V3SandwichUtility {
        ...
        returns (bytes memory payload, uint256 encodedValue)
    {
        ...
        uint8 jumpDest = SandwichCommon.getJumpDestFromSig(functionSignature);
     ...
     * @return payload Calldata bytes to execute backruns (empty tx.value because pool optimistically sends weth to sandwich contract)
     */
